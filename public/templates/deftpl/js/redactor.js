...
(function($)
{
	...
	RedactorPlugins.undo = {
        init: function ()
        {
            var button = this.buttonAdd('undo', 'Отменить изменения', this.undo);
        },
        undo: function()
        {
	...
	RedactorPlugins.redo = {
        init: function ()
        {
            var button = this.buttonAdd('redo', 'Повторить изменения', this.redo);
        },
        redo: function()
        {
	...
	RedactorPlugins.fontsize = {
		init: function()
		{
			...
			dropdown['remove'] = { title: 'Стандартный размер', callback: function() { that.resetFontsize(); } };

			this.buttonAdd('fontsize', 'Изменить размер шрифта', false, dropdown);
		},
		setFontsize: function(size)
		{
			this.blockSetStyle('font-size', size + 'px');
		},
	...
	RedactorPlugins.fontfamily = {
		init: function ()
		{
			...
			dropdown['remove'] = { title: 'Стандартный шрифт', callback: function() { that.resetFontfamily(); }};

			this.buttonAdd('fontfamily', 'Изменить шрифт', false, dropdown);
		},
		setFontfamily: function (value)
		{
			this.blockSetStyle('font-family', value);
		},
		resetFontfamily: function()
		{
			this.blockRemoveStyle('font-family');
		}
	};
	...
	RedactorPlugins.fontcolor = {
		...
		pickerSet: function(rule, type)
		{
			...
			this.$editor.focus();
			this.blockRemoveStyle(rule);
			if (type !== false) this.blockSetStyle(rule, type);
	...
	$.Redactor.opts = {
			...
			plugins: ['fontsize', 'fontfamily', 'fontcolor', 'undo', 'redo'],
			...
			buttons: ['undo', 'redo', 'html', 'formatting', 'bold', 'italic', 'deleted', 'unorderedlist', 'orderedlist',
					  'outdent', 'indent', 'image', 'video', 'file', 'table', 'link', 'alignment', '|',
					  'horizontalrule'], // 'underline', 'alignleft', 'aligncenter', 'alignright', 'justify'
			buttonsHideOnMobile: [],

			activeButtons: ['fll', 'flr', 'deleted', 'italic', 'bold', 'underline', 'unorderedlist', 'orderedlist',
			...
			formattingTags: ['p', 'blockquote', 'pre', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li'],
			...
			langs: {
				ru: {
					fll: 'Прижать влево (обтекать справа)',
					flr: 'Прижать вправо (обтекать слева)',
					html: 'Код',
					video: 'Видео',
					image: 'Изображение',
					table: 'Таблица',
		...
		fll: function(){
			...
			$.each(elements, $.proxy(function(i, elem)
			{
				...
				if($el.css('float') != 'left'){
					$el.css({float: 'left', display: 'block'});
				}else{
					$el.css({float: 'none', display: 'block'});
				}
		...
		flr: function(){
			...
			$.each(elements, $.proxy(function(i, elem)
			{
				...
				if($el.css('float') != 'right'){
					$el.css({float: 'right', display: 'block'});
				}else{
					$el.css({float: 'none', display: 'block'});
				}
